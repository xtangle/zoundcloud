...
module.exports = {
  ...
  'Adds a Download button for every track or playlist item in the Everything list': (browser) => {
    browser
      .waitForElementVisible('.searchResultGroupHeading')
      .elements('css selector', '.searchList__item .sound__body', (results) => {
        browser.globals.numSoundItemsInitial = results.value.length;
      })
      .elements('css selector', '.searchList__item .sound__body'
      ...
      .perform(() => {
        ...
        browser.assert.strictEqual(
          browser.globals.numZcBtnsInitial, browser.globals.numSoundItemsInitial,
          'Should add a Download button for every track or playlist item in the list.',
        );
      });
  },

  'Continues adding Download buttons as more items are loaded into the list': (browser) => {
    browser
      ...
      .perform(() => {
        ...
        browser.assert.strictEqual(
          browser.globals.numZcBtnsFinal, browser.globals.numSoundItemsFinal,
          'Should add a Download button for every track or playlist item in the list.',
        );
      });
  },

  'Adds a Download button for every track in the SoundCloud Go+ tracks list': (browser) => {
    browser
      .useXpath()
      ...
      .elements('css selector', '.searchList__item .sc-button-share + button.zc-button-download', (results) => {
  ...
  'Adds a Download button for every track in the Tracks list': (browser) => {
    browser
      .useXpath()
  ...
  'Adds a Download button for every album in the Albums list': (browser) => {
    browser
      .useXpath()
      ...
      .elements('css selector', '.searchList__item :not(.zc-button-download)'
        + ' + button.zc-button-download', (results) => {
        browser.globals.numZcBtns = results.value.length;
      })
      .perform(() => {
  ...
  'Adds a Download button for every playlist in the Playlists list': (browser) => {
    browser
      .useXpath()
  ...
  'Downloads the playlist when the Download button in the list is clicked': (browser) => {
