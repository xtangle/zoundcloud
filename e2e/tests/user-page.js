...
module.exports = {
  ...
  'Adds a Download button to a SoundCloud user page': (browser) => {
    browser
      .waitForElementVisible(zcBtnSelector)
      .assert.containsText(zcBtnSelector, 'Download');
  },

  'Downloads all tracks posted by the user when clicked': (browser) => {
  ...
  'Adds a Download button for every item posted by the user in the All tab': (browser) => {
    browser
      .waitForElementVisible('.userMain .soundList__item:first-of-type')
      .elements('css selector', '.userMain .soundList__item .sc-button-share + button.zc-button-download', (result) => {
  ...
  'Downloads the playlist when the Download button in the list is clicked': (browser) => {
    ...
    browser
      .click('.userMain .soundList__item div[aria-label="Playlist: Chapter Test by TEST USER"]'
        + ' button.zc-button-download')
      .assert.fileDownloaded(track1Path)
  ...
  'Adds a Download button for every track posted by the user in the Tracks tab': (browser) => {
    browser
      .useXpath()
  ...
  'Adds a Download button for every playlist posted by the user in the Playlists tab': (browser) => {
    browser
      .useXpath()
